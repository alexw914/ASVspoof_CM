function [tDCF_norm, CM_thresholds] = compute_tDCF(bonafide_score_cm, spoof_score_cm, Pfa_asv, Pmiss_asv, Pfa_spoof_asv, cost_model, print_cost)
...
if any([cost_model.Cfa cost_model.Cmiss cost_model.Cfa_spoof] < 0)
...
% Constants - see t-DCF journal paper
...
if print_cost
    ...
    fprintf('     Cfa         \t\t= %5.5f\t\t Cost of tandem system falsely accepting a nontarget\n', cost_model.Cfa);
    fprintf('     Cmiss       \t\t= %5.5f\t\t Cost of tandem system falsely rejecting target speaker\n', cost_model.Cmiss);
    fprintf('     Cfa_spoof   \t\t= %5.5f\t\t Cost of tandem sysmte falsely accepting spoof\n', cost_model.Cfa_spoof);
    fprintf('     Implied normalized t-DCF function (depends on t-DCF parameters and ASV errors), t_CM=CM threshold\n\n');
    fprintf('          tDCF_norm(t_CM) = %5.5f + %5.5f x Pmiss_cm(t_CM) + %5.5f x Pfa_cm(t_CM)\n', C0/tDCF_default, C1/tDCF_default, C2/tDCF_default);
    fprintf('\n');
    fprintf('     * The optimum value is given by the first term (%5.5f). This is the normalized t-DCF obtained with an error-free CM system.\n',C0/tDCF_default);
    fprintf('     * The minimum normalized cost (minimum over all possible thresholds) is always <= 1.00.\n');
    fprintf('\n');
